...
resource "google_secret_manager_secret_version" "database_url" {
  ...
  secret_data = replace(
    "postgresql://${google_sql_user.user.name}:${random_password.db_password.result}@${google_sql_database_instance.instance.ip_address.0.ip_address}:5432/${google_sql_database.database.name}",
    "%",
    "%%"
  )
}
